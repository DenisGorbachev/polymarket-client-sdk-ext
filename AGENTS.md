...
* Gamma `Market` is not a superset of CLOB `MarketResponse`. Several CLOB fields are missing or lossy in Gamma:
  * fpmm (FPMM contract address) exists in CLOB but there’s no equivalent address in Gamma; Gamma only has fpmm\_live
  * is\_50\_50\_outcome exists only in CLOB and is absent from Gamma’s field list
  * tokens: Vec<Token> with token\_id/outcome/price/winner is in CLOB, while Gamma only has separate outcomes, outcome\_prices, and clob\_token\_ids
    arrays (no winner)
  * rewards: Rewards (including rates) exists in CLOB; Gamma only provides rewards\_min\_size, rewards\_max\_spread, and optional clob\_rewards,
    which is a different shape
  * end\_date\_iso is Option\<DateTime<Utc>> in CLOB but only Option<NaiveDate> in Gamma, so time-of-day info can’t be represented
