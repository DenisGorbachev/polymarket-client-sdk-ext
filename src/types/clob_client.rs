...
impl ClobClient {
    ...
    pub async fn markets(&self, next_cursor: Option<String>) -> Result<Page<ClobMarketResponsePrecise>, ClobClientMarketsError> {
        ...
        let data = handle_iter!(
            data.into_iter()
                .filter(is_launched)
                .map(ClobMarketResponsePrecise::try_from),
            MarketTryFromFailed
        );
        Ok(Page::builder()
            .data(data)
    ...
    pub fn markets_stream(&self) -> impl Stream<Item = Result<Vec<ClobMarketResponsePrecise>, ClobClientMarketsError>> + '_ {
        self.get_markets_stream_at_cursor(NEXT_CURSOR_START.into())
    }

    pub fn get_markets_stream_at_cursor(&self, next_cursor: NextCursor) -> impl Stream<Item = Result<Vec<ClobMarketResponsePrecise>, ClobClientMarketsError>> + '_ {
